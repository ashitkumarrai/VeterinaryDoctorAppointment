...
public class DoctorDao {
	...
	try {	
			...
			java.sql.PreparedStatement ps=conn.prepareStatement("select * from doctors where email=? and password=?");
	...
	return status;
	 ...
	 public static int save(DocBean db){  
	        ...
	        try{  
	            ...
	            java.sql.PreparedStatement ps=con.prepareStatement("insert into doctors(docname,email,password,specialty,contact) values(?,?,?,?,?)");  	            
	    ...
	    public static int delete(int id){  
	        ...
	        try{  
	            Connection con=ConnectionProvider.getConnection();  
	            java.sql.PreparedStatement ps=con.prepareStatement("delete from doctors where id=?");  
	            ps.setInt(1,id);  
	            status=ps.executeUpdate();  
	    ...
	    public static DocBean getDoctor(int id,String email){  
	        ...
	        try{  
	            Connection con=ConnectionProvider.getConnection();  
	            java.sql.PreparedStatement ps=con.prepareStatement("select * from doctors where id=? and email=?");  
	            ps.setInt(1,id);  
	    ...
	    public static List<DocBean> getAllDoctors(){  
	        ...
	        try{  
	            Connection con=ConnectionProvider.getConnection();  
	            java.sql.PreparedStatement ps=con.prepareStatement("select * from doctors ORDER BY id");  
	            ResultSet rs=ps.executeQuery();  
	            while(rs.next()){  
